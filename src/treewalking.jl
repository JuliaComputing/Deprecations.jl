...
function Base.getindex(node::OverlayNode, idx::Integer)
    ...
    OverlayNode(node, expr, offset:(offset+expr.fullspan-1), offset - 1 + expr.span)
end
...
function Base.next(node::OverlayNode, state::Tuple{Int, Int})
    idx, offset = state
    expr = node.expr.args[idx]
    (OverlayNode(node, expr, offset:(offset+expr.fullspan-1), offset - 1 + expr.span), (idx+1, offset + expr.fullspan))
end
